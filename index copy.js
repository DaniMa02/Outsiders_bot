...
try {
  const botVars = getBotVariables();
  const hellChannelId = botVars.TESTP_CHANNEL; // o TESTP_CHANNEL

  if (!hellChannelId) {
    console.warn('⚠️ TEST_CHANNEL no está definido en bot_variables');
    console.log('📦 Variables actuales:', botVars);
    return;
  }

  const channel = await client.channels.fetch(hellChannelId);
  if (!channel) {
    console.warn('⚠️ Canal no encontrado:', hellChannelId);
    return;
  }
